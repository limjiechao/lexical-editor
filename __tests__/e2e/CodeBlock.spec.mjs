...
async function toggleCodeBlock(page, isCollab) {
  await click(page, isCollab, '.block-controls');
  await click(page, isCollab, '.dropdown .icon.code');
}
...
test.describe('CodeBlock', () => {
  ...
  test('Can create code block with markdown', async ({
    page,
    isRichText,
    isCollab,
    browserName,
  ...
  test('Can select multiple paragraphs and convert to code block', async ({
    ...
    await toggleCodeBlock(page, isCollab);
  ...
  test('Can switch highlighting language in a toolbar', async ({
    ...
    if (isRichText) {
      ...
      await click(page, isCollab, '.toolbar-item.code-language');
      await click(page, isCollab, 'button:has-text("SQL")');
      await assertHTML(
        page,
        isCollab,
        browserName,
  ...
  test('Can (un)indent multiple lines at once', async ({
    ...
    await page.keyboard.down('Shift');
    await click(page, isCollab, '.toolbar-item.alignment');
    await click(page, isCollab, 'button:has-text("Outdent")');
    await page.keyboard.up('Shift');
    await assertHTML(
      page,
      isCollab,
  ...
  test('Can move around lines with option+arrow keys', async ({
    ...
    await assertHTML(page, isCollab, browserName, abcHTML);
    await assertSelection(page, isCollab, endOfLastLine);
    await page.keyboard.press('ArrowUp');
    await page.keyboard.press('ArrowUp');
    // Workaround for #1173: just insert and remove a space to fix Firefox losing the selection
    await page.keyboard.type(' ');
    await page.keyboard.press('Backspace');
    await assertSelection(page, isCollab, endOfFirstLine);
    // End workaround
    ...
    await assertHTML(page, isCollab, browserName, bcaHTML);
    await page.keyboard.press('ArrowDown');
    await assertSelection(page, isCollab, endOfLastLine);
    await assertHTML(page, isCollab, browserName, bcaHTML);
  });
