  ...
  for (let i = 0; i < count; ++i) {
    ...
    if (triggersAndExpectations[i].markdownImport.length > 0) {
      test(`Should test importing markdown (${markdownText}) trigger.`, async ({
        ...
        await click(page, isCollab, 'i.markdown');

        const htmlInner = triggersAndExpectations[i].importExpectation;
        await assertHTML(page, isCollab, browserName, htmlInner);
...
async function assertMarkdownImportExport(
  ...
  await click(page, isCollab, '.action-button .markdown');
  await assertHTML(page, isCollab, browserName, expectedHTML, undefined, {
    ignoreClasses,
  });
  ...
  await click(page, isCollab, '.action-button .markdown');
  await click(page, isCollab, '.action-button .markdown');
  await assertHTML(page, isCollab, browserName, expectedHTML);
}
...
test.describe.parallel('Markdown', () => {
  ...
  BASE_BLOCK_SHORTCUTS.forEach((testCase) => {
    test(`can convert "${testCase.text}" shortcut`, async ({
      ...
      await assertHTML(page, isCollab, browserName, testCase.html, undefined, {
        ignoreClasses: true,
      });
  ...
  SIMPLE_TEXT_FORMAT_SHORTCUTS.forEach((testCase) => {
    test(`can convert "${testCase.text}" shortcut`, async ({
      ...
      await focusEditor(page, isCollab);
      await page.keyboard.type(testCase.text, {
        delay: legacyEvents ? 50 : 0,
      });
      await assertHTML(page, isCollab, browserName, testCase.html, undefined, {
        ignoreClasses: false,
      });
      await assertMarkdownImportExport(
        page,
        isCollab,
        browserName,
        testCase.text,
  ...
  test('can undo/redo nested transformations', async ({
    ...
    await assertHTML(page, isCollab, browserName, BOLD_ITALIC_STRIKETHROUGH);
    ...
    await assertHTML(page, isCollab, browserName, BOLD_ITALIC);
    await undo(page); // Undo transformation & its text typing
    await undo(page);
    await assertHTML(page, isCollab, browserName, BOLD);
    await undo(page); // Undo transformation & its text typing
    await undo(page);
    await assertHTML(page, isCollab, browserName, PLAIN);
  ...
  test('can convert markdown text into rich text', async ({
    page,
    isCollab,
    browserName,
  }) => {
    ...
    const originalHTML = await getHTML(page, isCollab);

    // Import from current markdown codeblock content
    await click(page, isCollab, '.action-button .markdown');
    await assertHTML(page, isCollab, browserName, IMPORTED_MARKDOWN_HTML);

    if (!isCollab) {
      await undo(page);
      await assertHTML(page, isCollab, browserName, originalHTML);
      await redo(page);
      await assertHTML(page, isCollab, browserName, IMPORTED_MARKDOWN_HTML);
  ...
  test('can type text with markdown', async ({page, isCollab, browserName}) => {
    await focusEditor(page, isCollab);
    await page.keyboard.type(TYPED_MARKDOWN);
    await assertHTML(page, isCollab, browserName, TYPED_MARKDOWN_HTML);
  });

  test('intraword text format', async ({page, isCollab, browserName}) => {
    await focusEditor(page, isCollab);
  ...
  test('can export text format next to a newline', async ({
    page,
    isCollab,
    browserName,
  }) => {
  ...
  test('can import single decorator node (#2604)', async ({
    page,
    isCollab,
    isCollabV2,
    browserName,
    ...
    await page.keyboard.type(
      '```markdown ![Yellow flower in tilt shift lens](' +
        sampleFlowerImageUrl(environment) +
        ')',
    );
    await click(page, isCollab, '.action-button .markdown');
  ...
  test('can import several text match transformers in a same line (#5385)', async ({
    ...
    await click(page, isCollab, '.action-button .markdown');
    await waitForSelector(page, isCollab, '.editor-image img');
    await waitForSelector(page, isCollab, '.editor-equation');
    await assertHTML(
      page,
      isCollab,
      browserName,
