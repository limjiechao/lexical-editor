...
test.describe.parallel('Links', () => {
  ...
  test(`Can convert a text node into a link`, async ({
    page,
    isCollab,
    browserName,
  }) => {
    ...
    await selectAll(page, isCollab, browserName);
    await setURL(page, isCollab, 'facebook.com');
  ...
  test(
    ...
    async ({page, isCollab, browserName}) => {
      ...
      await page.keyboard.press('Enter');
      // NOTE: Wait for the right collaborator to focus on the editor
      await sleep(1050);
  ...
  test.describe('Inserting text either side of links', () => {
    ...
    test.describe('Inserting text before links', () => {
      test.describe('Start-of-paragraph links', () => {
         ...
         */
        const setup = async (page, isCollab, browserName, insertMethod) => {
          await focusEditor(page, isCollab);
          ...
          if (insertMethod === 'type') {
            ...
            await pasteFromClipboard(page, isCollab, data);
          }
        ...
        test(`Can insert text before a start-of-paragraph link, via typing`, async ({
          ...
          await setup(page, isCollab, browserName, 'type');
        });
        ...
        test(`Can insert text before a start-of-paragraph link, via pasting plain text`, async ({
          ...
          await setup(page, isCollab, browserName, 'paste:plain');
        });
        ...
        test.skip(`Can insert text before a start-of-paragraph link, via pasting HTML`, async ({
          ...
          await setup(page, isCollab, browserName, 'paste:html');
        });
        ...
        test.skip(`Can insert text before a start-of-paragraph link, via pasting Lexical text`, async ({
          ...
          await setup(page, isCollab, browserName, 'paste:lexical');
        });
      });
  ...
  test('Can edit link with collapsed selection', async ({
    page,
    isCollab,
    browserName,
  }) => {
  ...
  test(`Can type text before and after`, async ({
    page,
    isCollab,
    browserName,
  }) => {
  ...
  test('Can create multiline links', async ({page, isCollab, browserName}) => {
    await focusEditor(page, isCollab);
    await page.keyboard.type('Hello world');
    await page.keyboard.press('Enter');
    await page.keyboard.type('Hello world');
  ...
  test('Can add, edit and remove links on images', async ({
    ...
    await click(page, isCollab, '.link');
    await focus(page, isCollab, '.link-input');
    ...
    // Edit link
    await click(page, isCollab, '.editor-image img');
    await click(page, isCollab, '.link-edit');
    await focus(page, isCollab, '.link-input');
    await selectAll(page, isCollab, browserName);
    await page.keyboard.press('Backspace');
    ...
    // Remove link
    await click(page, isCollab, '.editor-image img');
    await click(page, isCollab, '.link-trash');
  ...
  test('Can add, edit and remove links on multiple selected images', async ({
    ...
    // Select both images
    await click(page, isCollab, 'p:nth-child(1) .editor-image img');
    await page.keyboard.down('Shift');
    await click(page, isCollab, 'p:nth-child(2) .editor-image img');
...
async function setURL(page, isCollab, url) {
  await click(page, isCollab, '.link-edit');
  await focus(page, isCollab, '.link-input');
