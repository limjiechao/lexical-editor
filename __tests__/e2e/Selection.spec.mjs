...
test.describe.parallel('Selection', () => {
  ...
  test('does not focus the editor on load', async ({page, isCollab}) => {
  ...
  test('keeps single active selection for nested editors', async ({
    ...
    await insertImageCaption(page, isCollab, 'Hello world');
    ...
    await click(page, isCollab, 'header img');
    expect(await hasSelection('.image-caption-container')).toBe(true);
    expect(await hasSelection('.editor-shell')).toBe(false);
    ...
    if (browserName === 'firefox') {
      ...
      await click(page, isCollab, '.editor-shell', {
  ...
  test('can wrap post-linebreak nodes into new element', async ({
    ...
    await selectFromFormatDropdown(page, isCollab, '.code');
    await assertHTML(
      page,
      isCollab,
      browserName,
  ...
  test('can delete text by line backwards with CMD+delete', async ({
    ...
    await assertHTML(page, isCollab, browserName, lines('One', 'Two', '', ''));
    await page.keyboard.press('Backspace');
    await deleteLineBackward(page);
    await assertHTML(page, isCollab, browserName, lines('One', 'Two'));
    await deleteLineBackward(page);
    await assertHTML(page, isCollab, browserName, lines('One', ''));
    await page.keyboard.press('Backspace');
    await deleteLineBackward(page);
    await assertHTML(page, isCollab, browserName, lines(''));
  });
  ...
  test('can delete text by line forwards with opt+CMD+delete', async ({
    ...
    await assertHTML(page, isCollab, browserName, lines('One', 'Two', 'Three'));
    await deleteLineForward(page);
    await assertHTML(page, isCollab, browserName, lines('One', 'TwoThree'));
    await deleteLineForward(page);
    await assertHTML(page, isCollab, browserName, lines('One', 'Two'));
    await deleteLineForward(page);
    await assertHTML(page, isCollab, browserName, lines('One', 'Two'));
    ...
    await deleteLineForward(page);
    await assertHTML(page, isCollab, browserName, lines('', 'Two'));
    await deleteLineForward(page);
    await assertHTML(page, isCollab, browserName, lines('Two'));
    await deleteLineForward(page);
    await assertHTML(page, isCollab, browserName, lines(''));
    ...
    await pasteFromClipboard(page, isCollab, {
      'text/html': `
          <span class="editor-image" data-lexical-decorator="true" contenteditable="false">
            <div draggable="false">
              <img src="${sampleFlowerImageUrl(environment)}" alt="Yellow flower in tilt shift lens" draggable="false" style="height: inherit; max-width: 500px; width: inherit;">
            </div>
          </span>
        `,
    });
  ...
  test('Can delete at boundary #4221', async ({
    page,
    isPlainText,
    isCollab,
    browserName,
  ...
  test('Can select all with node selection', async ({
    page,
    isPlainText,
    isCollab,
    browserName,
  ...
  test(`Can't delete forward a Table`, async ({
    page,
    isPlainText,
    isCollab,
    browserName,
  ...
  test(`Can't delete backward a Table`, async ({
    page,
    isPlainText,
    isCollab,
    browserName,
  ...
  test('Can delete block elements', async ({
    page,
    isPlainText,
    isCollab,
    browserName,
  ...
  test('Can adjust triple click selection', async ({
    ...
    await click(page, isCollab, '.dropdown .item:has(.icon.h1)');
  ...
  test('Triple-clicking last cell in table should not select entire document', async ({
    ...
    await assertSelection(page, isCollab, expectedSelection);
  });
  ...
  test('Can persist the text style (color) from the paragraph', async ({
    ...
    test.skip(isPlainText);
    await focusEditor(page, isCollab);
    await click(page, isCollab, '.color-picker');
