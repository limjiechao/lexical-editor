...
test.describe.parallel('Tables', () => {
  test(`Can a table be inserted from the toolbar`, async ({
    ...
    await insertTable(page, isCollab, 2, 2);
  ...
  test.describe
    ...
    test(`Can exit the first cell of a table`, async ({
      ...
      await assertSelection(page, isCollab, {
        anchorOffset: 0,
        anchorPath: [1, ...hasWrapper(tableHorizontalScroll), 1, 0, 0],
        focusOffset: 0,
        focusPath: [1, ...hasWrapper(tableHorizontalScroll), 1, 0, 0],
      });
      ...
      await assertSelection(page, isCollab, {
        anchorOffset: 2,
        anchorPath: [1, ...hasWrapper(tableHorizontalScroll), 1, 0, 0, 0, 0],
        focusOffset: 2,
        focusPath: [1, ...hasWrapper(tableHorizontalScroll), 1, 0, 0, 0, 0],
      });

      await moveRight(page, 3);
      await assertSelection(page, isCollab, {
        anchorOffset: 0,
        anchorPath: [1, ...hasWrapper(tableHorizontalScroll), 2, 1, 0],
        focusOffset: 0,
        focusPath: [1, ...hasWrapper(tableHorizontalScroll), 2, 1, 0],
      });
    });
    ...
    test(`Can exit the last cell of a table`, async ({
      ...
      await assertSelection(page, isCollab, {
        anchorOffset: 2,
        anchorPath: [1, ...hasWrapper(tableHorizontalScroll), 2, 1, 0, 0, 0],
        focusOffset: 2,
        focusPath: [1, ...hasWrapper(tableHorizontalScroll), 2, 1, 0, 0, 0],
      });
  ...
  test(`Can't backwards delete from text to a table`, async ({
    ...
    test.skip(isPlainText || legacyEvents);
  ...
  test.describe.parallel(`Can navigate table with keyboard`, () => {
    test(`Can navigate cells horizontally`, async ({
      ...
      await assertSelection(page, isCollab, {
        anchorOffset: 0,
        anchorPath: [1, ...hasWrapper(tableHorizontalScroll), 1, 1, 0],
        focusOffset: 0,
        focusPath: [1, ...hasWrapper(tableHorizontalScroll), 1, 1, 0],
      });
    ...
    test('Should not navigate cells when typeahead menu is open and focused', async ({
      ...
      await waitForSelector(
        page,
        isCollab,
        `div[class="typeahead-popover component-picker-menu"] ul li:first-child.selected`,
      );
      ...
      await waitForSelector(
        page,
        isCollab,
        'div[class="typeahead-popover component-picker-menu"] ul li:nth-child(2).selected',
      );
    });
  });
  ...
  test(
    ...
    async ({
      ...
      await focusEditor(page, isCollab);
      await insertTable(page, isCollab, 2, 3);
  ...
  test(`Can select cells using Table selection via keyboard`, async ({
    ...
    await focusEditor(page, isCollab);
    await insertTable(page, isCollab, 3, 3);
  ...
  test(
    ...
    async ({
      ...
      await clickSelectors(page, isCollab, ['.bold', '.italic', '.underline']);

      await selectFromAdditionalStylesDropdown(
        page,
        isCollab,
        '.strikethrough',
      );
  ...
  test(`Can style on empty table cells and paragraphs with no text`, async ({
    ...
    await clickSelectors(page, isCollab, ['.bold']);

    // Add text after applying styles
    await click(page, isCollab, 'div[contenteditable="true"] p:first-of-type');
    await page.keyboard.type('abc');

    await click(page, isCollab, 'th p:first-of-type');
    await fillTablePartiallyWithText(page);
    ...
    async ({
      ...
      await withExclusiveClipboardAccess(async () => {
        ...
        await click(
          page,
          isCollab,
          'div.ContentEditable__root > p:first-of-type',
        );
        await click(
          page,
          isCollab,
  ...
  test(`Range Selection is corrected when it contains a partial Table.`, async ({
    ...
    await focusEditor(page, isCollab);
    await insertTable(page, isCollab, 1, 2);
  ...
  test('Can delete all with range selection anchored in table', async ({
    ...
    await focusEditor(page, isCollab);
    await insertTable(page, isCollab, 1, 1);
  ...
  test(`Horizontal rule inside cell`, async ({
    ...
    await insertHorizontalRule(page, isCollab);
  ...
  test(
    ...
    async ({
      ...
      await click(
        page,
        isCollab,
        '.PlaygroundEditorTheme__tableCell:first-child',
      );
      ...
      await waitForSelector(page, isCollab, '.editor-image img');
  ...
  test('Resize merged cells height', async ({
    ...
    test.skip(isPlainText);
    test.fixme(isCollab && IS_LINUX && browserName === 'firefox');
    await initialize({
      browserName,
      environment,
      isCollab,
  ...
  test('Merge/unmerge cells (1)', async ({
    ...
    await insertTable(page, isCollab, 1, 3);
  ...
  test('Merged cell tab navigation reverse', async ({
    ...
    await click(
      page,
      isCollab,
      ':nth-match(.PlaygroundEditorTheme__tableCell, 7)',
    );
    for (const i of Array.from({length: 9 - 2}, (_v, idx) => idx)) {
      await page.keyboard.type(String(i));
      await page.keyboard.down('Shift');
  ...
  test('Insert row above (with conflicting merged cell)', async ({
    ...
    await moveLeft(page, 1);
    await insertTableRowBelow(page, isCollab);
  ...
  test('Insert column before (with conflicting merged cell)', async ({
    ...
    await moveDown(page, 1);
    await moveRight(page, 1);
    await insertTableColumnBefore(page, isCollab);
  ...
  test(
    ...
    async ({
      ...
      await insertTable(page, isCollab, 4, 2);
      ...
      await deleteTableRows(page, isCollab);
  ...
  test('Delete selected rows (with merged cell overflowing selection from top and bottom)', async ({
    ...
    await insertTable(page, isCollab, 10, 5);
  ...
  test('Delete columns (with conflicting merged cell)', async ({
    ...
    await insertTable(page, isCollab, 2, 4);
    ...
    await deleteTableColumns(page, isCollab);
  ...
  test('Background color to cell', async ({
    environment,
    page,
    isPlainText,
    isCollab,
    ...
    await insertTable(page, isCollab, 1, 1);
    await setBackgroundColor(page, isCollab);
    await click(page, isCollab, '.color-picker-basic-color button');
    await click(page, isCollab, '.Modal__closeButton');
  ...
  test('Cell merge feature disabled', async ({
    environment,
    page,
    isPlainText,
    isCollab,
  ...
  test('Paste and insert new lines after unmerging cells', async ({
    ...
    await withExclusiveClipboardAccess(async () => {
      ...
      await click(page, isCollab, '.PlaygroundEditorTheme__paragraph');
  ...
  test('Can insert multiple rows above the selection', async ({
    ...
    await insertTableRowAbove(page, isCollab);
  ...
  test.describe('with context menu', () => {
    ...
    test(`Can select cells using Table selection and cut them with the context menu`, async ({
      ...
      await withExclusiveClipboardAccess(async () => {
        await click(page, isCollab, 'div[contenteditable] th p', {
          button: 'right',
        });
  ...
  test(`Table action menu is hidden when cell overflows`, async ({
    ...
    await click(page, isCollab, 'th >> nth=1');
    ...
    // Click the cell
    await click(page, isCollab, 'th >> nth=0');
  ...
  test(`Can expand table to fit content when pasting table into table`, async ({
    ...
    await pasteFromClipboard(page, isCollab, {
      'text/html': TABLE_WITH_MERGED_CELLS,
    });
    ...
    await withExclusiveClipboardAccess(async () => {
      ...
      await selectCellFromTableCoord(page, isCollab, {x: 0, y: 2});
    ...
    await withExclusiveClipboardAccess(async () => {
      ...
      await selectCellFromTableCoord(page, isCollab, {x: 2, y: 1});
  ...
  test(`Can paste table containing merged cells into table`, async ({
    ...
    await withExclusiveClipboardAccess(async () => {
      ...
      await selectCellFromTableCoord(page, isCollab, {x: 0, y: 0});
