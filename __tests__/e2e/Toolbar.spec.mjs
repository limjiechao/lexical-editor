...
test.describe('Toolbar', () => {
  ...
  test(
    ...
    async ({page, isPlainText, isCollab, browserName, environment}) => {
      ...
      await focus(page, isCollab, '.ImageNode__contentEditable');
      ...
      // Add table
      await selectFromInsertDropdown(page, isCollab, '.table');
      await click(
        page,
        isCollab,
        '[data-test-id="table-model-confirm-insert"] button',
      );
  ...
  test('Center align image', async ({
    ...
    await focus(page, isCollab, '.editor-image');
    await page.pause();
