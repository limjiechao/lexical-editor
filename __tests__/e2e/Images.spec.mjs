...
test.describe('Images', () => {
  ...
  test(`Can create a decorator and move selection around it`, async ({
    ...
    await click(page, isCollab, 'div[contenteditable="true"]');
  ...
  test('Can add images by arbitrary URL', async ({
    ...
    await insertUrlImage(page, isCollab, SAMPLE_SVG_URL, 'lexical logo');
  ...
  test('Can be dragged and dropped correctly when the image is clicked', async ({
    ...
    if (browserName === 'firefox') {
      ...
      await dragImage(page, isCollab, 'span[data-lexical-text="true"]');
  ...
  test('Select image, then select text - EditorState._selection updates with mousedown #2901', async ({
    ...
    const lexicalSelection = await evaluate(page, isCollab, (editor) => {
  ...
  test('Node selection: can select multiple image nodes and replace them with a new image', async ({
    ...
    test.fixme(legacyEvents && browserName === 'webkit');
    test.skip(isPlainText);
    ...
    await page.keyboard.press('Enter');
    await insertSampleImage(page, isCollab, 'alt');
    await page.keyboard.press('Enter');
          ...
          <span
            ...
            <div draggable="false">
              <img
                ...
                draggable="false"
                src="${sampleLandscapeImageUrl(environment)}"
                style="height: inherit; max-width: 500px; width: inherit" />
            </div>
          </span>
          <br />
